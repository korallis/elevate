...
while IFS= read -r -d '' mdfile; do
  ...
  if [[ -n "$running_script_abs" ]]; then
    target_abs="$(cd "$ROOT_DIR" && python3 -c 'import os,sys;print(os.path.abspath(sys.argv[1]))' "$rel_target" 2>/dev/null || true)"
    if [[ "$target_abs" == "$running_script_abs" ]]; then
...
if [[ -f "$ROOT_DIR/package.json" ]]; then
  echo "Installing root dev dependencies with pnpm (via corepack if available)…"
  if command -v corepack >/dev/null 2>&1; then
    corepack enable || true
    if (cd "$ROOT_DIR" && pnpm install); then
      :
    else
      echo "pnpm install failed; you can run 'corepack prepare pnpm@<version> --activate' then 'pnpm install' later."
    fi
  elif command -v pnpm >/dev/null 2>&1; then
    if (cd "$ROOT_DIR" && pnpm install); then
      :
    else
      echo "pnpm install failed; install pnpm v9 via corepack and retry."
    fi
  else
