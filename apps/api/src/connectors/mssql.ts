...
export class MSSQLConnector extends BaseConnector {
  ...
  readonly name = 'Microsoft SQL Server';
  ...
  async testConnection(config: AuthConfig): Promise<ConnectionTestResult> {
    ...
    try {
      this.validateConfig(config, ['server', 'user', 'password']);
  ...
  async listDatabases(): Promise<DatabaseInfo[]> {
    const result = await this.executeQuery(`
      SELECT name 
      FROM sys.databases 
      WHERE name NOT IN ('master', 'tempdb', 'model', 'msdb', 'Resource', 'ReportServer', 'ReportServerTempDB')
      ORDER BY name
    `);
  ...
  async executeQuery(sql: string, params?: unknown[]): Promise<QueryResult> {
    if (!this.connected || !this.pool) {
      throw new ConnectionError('Not connected to SQL Server');
    }
  ...
  private createPool(config: AuthConfig): sql.ConnectionPool {
    const poolConfig: sql.config = {
      ...
      options: {
        encrypt: config.credentials.encrypt !== false, // Default to true for Azure
  ...
  protected mapColumnType(sqlServerType: string): string {
    const type = sqlServerType.toLowerCase();
